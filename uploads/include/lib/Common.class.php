...
class Common {
	...
	public static function exitWithMessage($message_detail, $forward_url, $second = 3,$type="message") {
		...
		if ($type == 'error') {
			// 返回页面为 上一页
			$message_detail = "$message_detail <script>setTimeout(\"window.history.go(-1);\", " . ($second * 1000) . ");</script>";
		} else {
			// 自定义 返回页面
	...
	// 表单提交后检查过滤
	public static function checkFormFilter($data) {
		if( empty($data) ) {
			return false;
		}
	...
	// 打印信息
	public static function prt($data) {
	...
	// 打印信息并结束脚本
	public static function prtExit($data) {
