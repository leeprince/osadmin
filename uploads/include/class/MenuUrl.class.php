...
class MenuUrl extends Base
{
	...
	const SESSION_NAME = 'ygx_menuurl_list';
	...
	// 通过 father_id 父模块找到子模块
	public static function getMenuChirldByFatherID($father_id) {

		if ( ! $father_id || ! is_numeric($father_id)) {
			return array();
		}
		$condition = array();
		$condition = array("father_menu" => $father_id);

		$columns = array('menu_id', 'menu_name');

		$db = self::__instance();
		$res = $db->select(self::getTableName(), $columns, $condition);
